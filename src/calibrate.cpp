...
int calibrate(ConfigParser config) {
    int numBoards = config.getInt("numBoards");
    int board_w = config.getInt("board_w");
    int board_h = config.getInt("board_h");
    ...
    const char* v = config.getString("video1");
    VideoCapture *vid;
    if (is_number(v)) {
        int d;
        sscanf(v, "%d", &d);
